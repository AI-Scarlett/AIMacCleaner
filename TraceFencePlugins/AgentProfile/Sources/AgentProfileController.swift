...
private actor AgentProfileDiagnosticEngine {
    func run(configuration: AgentProfileConfiguration) async -> AgentProfileDiagnosticSnapshot {
        ...
        let antigravityRoute = AgentProfileAntigravityRouteInspector.inspect()
...
/// Reads only the local `agy` entrypoint. This distinguishes the proxy policy
/// generated by Agent Profile from a separate launcher that overrides it.
/// No provider request, token, or credential is read here.
enum AgentProfileAntigravityRouteInspector {
    static func inspect(
        commandURL: URL = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".local/bin/agy")
    ) -> AgentProfileAntigravityRouteSnapshot {
        guard FileManager.default.fileExists(atPath: commandURL.path) else {
            return .empty
        }
        guard let text = try? String(contentsOf: commandURL, encoding: .utf8) else {
            return AgentProfileAntigravityRouteSnapshot(
                status: .unreadableLauncher,
                commandPath: commandURL.path,
                details: "The Antigravity CLI entrypoint could not be inspected."
            )
        }
        return classify(commandPath: commandURL.path, launcherText: text)
    }
    ...
    static func classify(
        ...
        let clearsProxy = launcherText.range(
            of: #"unset\s+(?:HTTP_PROXY|HTTPS_PROXY|ALL_PROXY)"#,
            options: .regularExpression
        ) != nil
        let overridesCloudCodeEndpoint = launcherText.range(
            of: #"(?:export\s+)?CLOUD_CODE_URL\s*="#,
            options: .regularExpression
        ) != nil

        if overridesCloudCodeEndpoint {
            return AgentProfileAntigravityRouteSnapshot(
                status: .endpointOverridden,
                commandPath: commandPath,
                details: clearsProxy
                    ? "The CLI launcher overrides the Cloud Code endpoint and clears proxy variables before launch."
                    : "The CLI launcher overrides the Cloud Code endpoint before launch."
            )
        }
        if clearsProxy {
            return AgentProfileAntigravityRouteSnapshot(
                status: .proxySuppressedByLauncher,
                commandPath: commandPath,
                details: "The CLI launcher clears the proxy variables generated by Agent Profile."
            )
        }
        return AgentProfileAntigravityRouteSnapshot(
            status: .officialPath,
            commandPath: commandPath,
            details: "No local endpoint override or proxy-variable clearing was found in the CLI entrypoint."
        )
    }
}
...
final class AgentProfileController: ObservableObject {
    ...
    @Published private(set) var antigravityRoute = AgentProfileAntigravityRouteSnapshot.empty
    ...
    init(context: PluginRuntimeContext) {
        ...
        self.antigravityRoute = self.diagnostic.antigravityRoute
            ?? AgentProfileAntigravityRouteInspector.inspect()
    ...
    func runDiagnostics() {
        ...
        diagnosticTask = Task { [weak self] in
            ...
            diagnostic = result
            antigravityRoute = result.antigravityRoute
                ?? AgentProfileAntigravityRouteInspector.inspect()
            isDiagnosing = false
    ...
    func refreshAntigravityRoute() {
        let route = AgentProfileAntigravityRouteInspector.inspect()
        antigravityRoute = route
        diagnostic.antigravityRoute = route
        objectWillChange.send()
    }
    ...
    private static func diagnosticReport(
        ...
        antigravityRoute: AgentProfileAntigravityRouteSnapshot
    ) -> String {
        """
        ...
        Antigravity CLI route: \(antigravityRoute.status.rawValue)
        Antigravity route details: \(antigravityRoute.details)
