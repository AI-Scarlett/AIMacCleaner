...
final class AgentProfilePolicyTests: XCTestCase {
    ...
    func testGeneratedProfileContainsUniversalWrappersLaunchersAndBrowserExtension() throws {
        ...
        let environment = try String(contentsOf: assets.environmentURL, encoding: .utf8)
        XCTAssertFalse(environment.contains("export TZ="), "CLI and desktop launchers must inherit the macOS timezone.")
        XCTAssertTrue(environment.contains("export TRACEFENCE_TIMEZONE="), "Timezone remains profile metadata for the browser extension.")
