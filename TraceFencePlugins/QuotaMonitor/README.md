...
`1.0.9` 在插件内注册一个原生 Control Strip 项目，不依赖 BetterTouchTool、Node、Shell Script
Widget 或自动化权限。它只显示一个 Agent，而不是把所有 Provider 合并成一条红色长条：
...
升级自 `1.0.8` 且仍有旧 BTT 控件时，插件面板会出现一次“清理旧版”操作。该迁移只会删除
此插件曾写入且仍记录在本机状态文件中的 UUID，不读取、更改或导出用户的其他 BTT 自动化规则。
日常运行完全不需要 BTT。
