...
extension Localizer {
    ...
    var aiSettingsDesc: String { t("默认使用 Apple Intelligence 本机模型分析，不需要也不支持用户配置外部大模型。", en: "Uses Apple Intelligence on-device by default. External model configuration is not required or supported in this build.", zhHant: "預設使用 Apple Intelligence 本機模型分析，不需要也不支援使用者設定外部大模型。", ja: "既定ではApple Intelligenceのオンデバイスモデルを使用します。このビルドでは外部モデル設定は不要で、対応していません。", ko: "기본적으로 Apple Intelligence 온디바이스 모델을 사용합니다. 이 빌드에서는 외부 모델 설정이 필요하지 않으며 지원되지 않습니다.", mt: "Uses Apple Intelligence on-device by default. External model configuration is not required or supported in this build.") }
    ...
    var configureAPIKeyFirst: String { t("此版本默认使用 Apple Intelligence，不需要 API Key。", en: "This build uses Apple Intelligence by default and does not require an API key.", zhHant: "此版本預設使用 Apple Intelligence，不需要 API Key。", ja: "このビルドは既定でApple Intelligenceを使用し、API Keyは不要です。", ko: "이 빌드는 기본적으로 Apple Intelligence를 사용하며 API Key가 필요 없습니다.", mt: "This build uses Apple Intelligence by default and does not require an API key.") }
    ...
    var localAnalysisNoExternalAI: String { t("默认使用 Apple Intelligence 本机分析，不会连接第三方 AI 服务。", en: "Apple Intelligence is used on-device by default and does not connect to third-party AI services.", zhHant: "預設使用 Apple Intelligence 本機分析，不會連接第三方 AI 服務。", ja: "既定ではApple Intelligenceによるオンデバイス分析を使用し、第三者AIサービスには接続しません。", ko: "기본적으로 Apple Intelligence 온디바이스 분석을 사용하며 타사 AI 서비스에 연결하지 않습니다.", mt: "Apple Intelligence is used on-device by default and does not connect to third-party AI services.") }
