...
extension Localizer {
    ...
    var selectMonitorDirs: String { t("请选择主目录或 Agent 数据目录（例如 ~/.codex、~/.claude、~/.grok，也可按住 ⌘ 多选需要监控的项目目录）", en: "Select your Home folder or agent data folders such as ~/.codex, ~/.claude, and ~/.grok. Hold ⌘ to select multiple project folders.", zhHant: "請選擇主目錄或 Agent 資料目錄（例如 ~/.codex、~/.claude、~/.grok，也可按住 ⌘ 多選需要監控的專案目錄）", ja: "ホームフォルダ、または ~/.codex、~/.claude、~/.grok などのAgentデータフォルダを選択してください。⌘キーで複数のプロジェクトフォルダも選択できます。", ko: "홈 폴더 또는 ~/.codex, ~/.claude, ~/.grok 같은 Agent 데이터 폴더를 선택하세요. ⌘ 키로 여러 프로젝트 폴더를 선택할 수 있습니다.", mt: "Select your Home folder or agent data folders such as ~/.codex, ~/.claude, and ~/.grok. Hold ⌘ to select multiple project folders.") }
