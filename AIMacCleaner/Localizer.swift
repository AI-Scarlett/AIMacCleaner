...
extension Localizer {
    ...
    var aiSettingsDesc: String { t("默认使用 Apple Intelligence 本机模型分析，不需要外部账号；也可以选择填写兼容 OpenAI 的模型服务。", en: "Uses Apple Intelligence on-device by default, with no external account required. You can optionally configure an OpenAI-compatible provider.", zhHant: "預設使用 Apple Intelligence 本機模型分析，不需要外部帳號；也可以選擇填寫相容 OpenAI 的模型服務。", ja: "既定ではApple Intelligenceのオンデバイスモデルを使用します。外部アカウントは不要で、OpenAI互換プロバイダも任意で設定できます。", ko: "기본적으로 Apple Intelligence 온디바이스 모델을 사용하며 외부 계정이 필요 없습니다. OpenAI 호환 제공자를 선택적으로 설정할 수 있습니다.", mt: "Uses Apple Intelligence on-device by default, with no external account required. You can optionally configure an OpenAI-compatible provider.") }
    ...
    var configureAPIKeyFirst: String { t("请先配置 API Key，或切回 Apple Intelligence。", en: "Please configure an API key first, or switch back to Apple Intelligence.", zhHant: "請先設定 API Key，或切回 Apple Intelligence。", ja: "先にAPI Keyを設定するか、Apple Intelligenceに戻してください。", ko: "먼저 API Key를 설정하거나 Apple Intelligence로 다시 전환하세요.", mt: "Please configure an API key first, or switch back to Apple Intelligence.") }
