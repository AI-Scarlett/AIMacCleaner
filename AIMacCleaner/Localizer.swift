...
extension Localizer {
    ...
    var aiSettingsDesc: String { t("默认先使用 Apple Intelligence；不可用时可按你的设置回退到第三方 OpenAI 兼容模型。", en: "Uses Apple Intelligence first by default, then can fall back to your configured OpenAI-compatible provider if Apple is unavailable.", zhHant: "預設先使用 Apple Intelligence；不可用時可依你的設定回退到第三方 OpenAI 相容模型。", ja: "既定では Apple Intelligence を優先し、利用できない場合は設定済みの OpenAI 互換プロバイダーへフォールバックできます。", ko: "기본적으로 Apple Intelligence를 먼저 사용하고 사용할 수 없으면 설정한 OpenAI 호환 제공자로 전환할 수 있습니다.", mt: "Uses Apple Intelligence first by default, then can fall back to your configured OpenAI-compatible provider if Apple is unavailable.") }
    var appReviewDemoMode: String { t("Apple Intelligence 本机分析", en: "Apple Intelligence On-device Analysis", zhHant: "Apple Intelligence 本機分析", ja: "Apple Intelligence オンデバイス分析", ko: "Apple Intelligence 온디바이스 분석", mt: "Apple Intelligence On-device Analysis") }
    var appReviewDemoModeDesc: String { t("目录摘要会在本机交给 Apple Intelligence 分析，不需要上传到第三方模型服务。", en: "Directory summaries are analyzed on this Mac with Apple Intelligence, without sending them to a third-party model service.", zhHant: "目錄摘要會在本機交給 Apple Intelligence 分析，不需要上傳到第三方模型服務。", ja: "ディレクトリ要約はこのMac上のApple Intelligenceで分析され、第三者モデルサービスには送信されません。", ko: "디렉터리 요약은 이 Mac의 Apple Intelligence로 분석되며 타사 모델 서비스로 전송되지 않습니다.", mt: "Directory summaries are analyzed on this Mac with Apple Intelligence, without sending them to a third-party model service.") }
    ...
    var labFeaturesOverview: String { t("当前实验室包含以下功能：", en: "The current Lab includes these features:", zhHant: "目前實驗室包含以下功能：", ja: "現在のラボには次の機能が含まれます。", ko: "현재 실험실에는 다음 기능이 포함됩니다.", mt: "The current Lab includes these features:") }
    ...
    var configureAPIKeyFirst: String { t("如需使用第三方模型，请在 AI 设置里填写 API Key。默认会先尝试 Apple Intelligence。", en: "To use an external model, enter an API key in AI Settings. Apple Intelligence is tried first by default.", zhHant: "如需使用第三方模型，請在 AI 設定裡填寫 API Key。預設會先嘗試 Apple Intelligence。", ja: "外部モデルを使う場合は AI 設定で API Key を入力してください。既定では Apple Intelligence を先に試します。", ko: "외부 모델을 사용하려면 AI 설정에서 API Key를 입력하세요. 기본적으로 Apple Intelligence를 먼저 시도합니다.", mt: "To use an external model, enter an API key in AI Settings. Apple Intelligence is tried first by default.") }
    var appReviewDemoStatus: String { t("正在使用 Apple Intelligence 分析本机目录...", en: "Analyzing local directories with Apple Intelligence...", zhHant: "正在使用 Apple Intelligence 分析本機目錄...", ja: "Apple Intelligenceでローカルディレクトリを分析中...", ko: "Apple Intelligence로 로컬 디렉터리를 분석 중...", mt: "Analyzing local directories with Apple Intelligence...") }
    ...
    var localAnalysisNoExternalAI: String { t("默认优先使用 Apple Intelligence；只有你配置并确认第三方模型时，才会发送必要元数据。", en: "Apple Intelligence is preferred by default. Necessary metadata is sent only when you configure and confirm an external model.", zhHant: "預設優先使用 Apple Intelligence；只有你配置並確認第三方模型時，才會傳送必要元資料。", ja: "既定では Apple Intelligence を優先します。外部モデルを設定して確認した場合のみ、必要なメタデータを送信します。", ko: "기본적으로 Apple Intelligence를 우선 사용합니다. 외부 모델을 설정하고 확인한 경우에만 필요한 메타데이터를 전송합니다.", mt: "Apple Intelligence is preferred by default. Necessary metadata is sent only when you configure and confirm an external model.") }
    ...
    func localImpactSummary(appName: String, risk: String, type: String, isSafe: Bool) -> String {
        switch type {
        case "app":
            return t("本地：\(appName) 是已安装应用。TraceFence 仅展示清单和缓存上下文；App Store 版本已禁用卸载操作。风险：\(risk)。",
                     en: "Local: \(appName) is an installed app. TraceFence shows inventory and cache context only; uninstall actions are disabled in this App Store build. Risk: \(risk).",
                     zhHant: "本地：\(appName) 是已安裝應用。TraceFence 僅展示清單和快取上下文；App Store 版本已停用解除安裝操作。風險：\(risk)。",
                     ja: "ローカル：\(appName) はインストール済みアプリです。TraceFenceは一覧とキャッシュ情報のみを表示し、このApp Store版ではアンインストール操作は無効です。リスク：\(risk)。",
                     ko: "로컬: \(appName)은 설치된 앱입니다. TraceFence는 목록과 캐시 컨텍스트만 표시하며 이 App Store 빌드에서는 제거 작업이 비활성화되어 있습니다. 위험: \(risk).",
                     mt: "Local: \(appName) is an installed app. TraceFence shows inventory and cache context only; uninstall actions are disabled in this App Store build. Risk: \(risk).")
        case "dependency":
