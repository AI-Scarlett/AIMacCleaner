    ...
    private var quotaEmptyStateCard: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            ...
            if quotaService.quotaPluginNeedsInstallation {
                Button {
                    TraceFencePluginPresentationCenter.shared.openStore()
                    openMainConsole()
                } label: {
                    Label(
                        localizer.t("前往插件商城安装", en: "Install from Plugin Store", zhHant: "前往外掛商城安裝", ja: "プラグインストアからインストール", ko: "플러그인 스토어에서 설치", mt: "Install from Plugin Store"),
