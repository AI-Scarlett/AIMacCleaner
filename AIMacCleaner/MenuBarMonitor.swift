            ...
            VStack(alignment: .leading, spacing: Theme.Spacing.md) {
                HStack(alignment: .top, spacing: Theme.Spacing.md) {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(localizer.t("读取用户授权的本地 Agent 状态，显示 5 小时、周/月额度和手动重置次数。", en: "Reads user-authorized local agent status to show 5-hour, weekly/monthly quota windows and manual reset credits.", zhHant: "讀取使用者授權的本機 Agent 狀態，顯示 5 小時、週/月額度與手動重置次數。", ja: "ユーザーが許可したローカル Agent 状態を読み取り、5 時間、週/月クォータと手動リセット回数を表示します。", ko: "사용자가 승인한 로컬 Agent 상태를 읽어 5시간, 주간/월간 할당량과 수동 재설정 횟수를 표시합니다.", mt: "Reads user-authorized local agent status to show 5-hour, weekly/monthly quota windows and manual reset credits."))
                            .font(Theme.Font.caption)
        ...
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            HStack(alignment: .top, spacing: Theme.Spacing.md) {
                ...
                VStack(alignment: .leading, spacing: Theme.Spacing.xs) {
                    ...
                    Text(localizer.t(
                        "TraceFence 会在用户打开额度页后读取本机已授权的 Agent 状态，显示 5 小时、周/月额度和手动重置次数。首次读取可能需要几秒；如果缺少登录、API Key 或数据目录授权，结果会在这里给出处理建议。",
                        en: "TraceFence reads user-authorized local agent status when this quota page opens, showing 5-hour, weekly/monthly quota windows and manual reset credits. The first read can take a few seconds; missing login, API key, or data-folder authorization will appear here with next steps.",
                        zhHant: "TraceFence 會在使用者打開額度頁後讀取本機已授權的 Agent 狀態，顯示 5 小時、週/月額度與手動重置次數。首次讀取可能需要幾秒；如果缺少登入、API Key 或資料夾授權，結果會在這裡給出處理建議。",
                        ja: "TraceFence はこのクォータページを開いたときに、ユーザーが許可したローカル Agent 状態を読み取り、5 時間、週/月クォータと手動リセット回数を表示します。初回読み取りには数秒かかる場合があり、ログイン、API キー、データフォルダ許可が不足している場合は対処方法を表示します。",
                        ko: "TraceFence는 이 할당량 페이지가 열릴 때 사용자가 승인한 로컬 Agent 상태를 읽어 5시간, 주간/월간 할당량과 수동 재설정 횟수를 표시합니다. 첫 읽기에는 몇 초가 걸릴 수 있으며 로그인, API Key, 데이터 폴더 승인 문제가 있으면 여기에서 처리 방법을 안내합니다.",
                        mt: "TraceFence reads user-authorized local agent status when this quota page opens, showing 5-hour, weekly/monthly quota windows and manual reset credits. The first read can take a few seconds; missing login, API key, or data-folder authorization will appear here with next steps."))
                        .font(Theme.Font.caption)
                        .foregroundStyle(Theme.Colors.textSecondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
