...
struct VerifyStorageOptimizer {
    static func main() throws {
        ...
        try expect(first.summary.agentSessionFileCount == 2, "expected two Agent session files")
        try expect(first.summary.hashedFileCount == 2, "first scan should hash both Agent files")
        try expect(first.summary.cacheHitCount == 0, "first scan should have no cache hits")
        try expect(first.summary.embeddedMediaOccurrences == 4, "expected four embedded media occurrences")
        try expect(first.summary.uniqueMediaCount == 2, "expected two unique media payloads")
        try expect(
        ...
        try expect(second.summary.cacheHitCount == 2, "second scan should reuse both fingerprints")
        ...
        try expect(third.summary.cacheHitCount == 1, "append scan should reuse the unchanged session")
        ...
        try expect(fourth.summary.cacheHitCount == 1, "rewrite scan should still reuse the unchanged session")
        try expect(fourth.summary.hashedFileCount == 1, "rewrite scan should re-read the changed file")
        try expect(fourth.summary.fingerprintedBytes == Int64(replacement.utf8.count), "rewrite continuity guard failed to force a full re-read")

        print("STORAGE_OPTIMIZER_OK sessions=2 duplicate=\(first.summary.duplicateMediaBytes) history=\(historical.count) installers=\(installers.count) cache_hits=\(second.summary.cacheHitCount) append_bytes=\(third.summary.fingerprintedBytes) rewrite_bytes=\(fourth.summary.fingerprintedBytes)")
    }
